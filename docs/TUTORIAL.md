...
```bash
poetry run python src/demos/demo_shear_thinning.py
```

### Task 4.2: Animation Creation
Follow our new guide to create your first CFD video:
1. Generate frames: `poetry run python src/demos/make_animation_frames.py`
2. Encode video: `ffmpeg -framerate 10 -i src/results/make_animation_frames/frames/frame_%04d.png -c:v libx264 -pix_fmt yuv420p src/results/make_animation_frames/cavity_simulation.mp4`
