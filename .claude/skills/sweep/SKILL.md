...
> **注:** 手動でステージングしたケース群を複数マシンに分担させて実行する場合は、このスキル（factor 展開・単一マシン）ではなく `./bin/sweep` CLI を使う（マニフェスト: `configs/lbm_dem/sweeps/manifests/`、使い方: README の「Multi-Machine Sweeps」節）。
