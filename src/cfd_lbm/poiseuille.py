...
Geometry (y-direction):
    y = 0        : bottom wall  (bounce-back, no-slip at y = 0.5)
    y = 1..ny-2  : fluid nodes
    y = ny-1     : top wall     (bounce-back, no-slip at y = ny-1.5)

    Effective channel height : H_eff = ny - 2  (lattice units)

Analytical solution (parabolic Hagen-Poiseuille profile):
    u(xi) = Fx / (2 nu) * xi * (H - xi)
    where xi = y - 0.5  (bounce-back offset),  u_max = Fx*H^2 / (8*nu)
...
References:
  ...
  - Krueger et al. (2017). The Lattice Boltzmann Method.
"""
...
def guo_force(
    ...
    Compute Guo et al. (2002) discrete forcing term F_i [Q, nx, ny].
        ...
        F_i = w_i (1 - w/2) [(e_i - u)/cs^2 + (e_i*u) e_i / cs^4] * F
...
class PoiseuilleFlow:
    """
    2D D2Q9 LBM solver for body-force-driven Poiseuille channel flow.
    ...
    def __init__(
        ...
        Parameters
        ----------
        nx    : channel length (periodic; 8 is sufficient)
        ny    : total grid height including walls (fluid = ny-2 layers)
        Re    : Reynolds number  Re = u_max * H / nu,  H = ny-2
    ...
    def _macroscopic(self) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Density and velocity; Guo velocity correction at fluid nodes."""
        ...
        # Guo correction only at fluid nodes: u_true = u_stream + F*dt/(2*rho)
        # Solid nodes must keep ux=0 so their feq doesn't corrupt streaming.
        fluid = ~self.solid
    ...
    def _collide(self, rho: np.ndarray, ux: np.ndarray, uy: np.ndarray) -> None:
        ...
        values so that streaming correctly propagates those values to the
        adjacent fluid nodes. Applying BGK at solid nodes would overwrite
        the bounce-back state and introduce a spurious slip velocity.
        """
    ...
    def analytical_u(self, y: np.ndarray) -> np.ndarray:
        ...
        Parameters
        ----------
        y : node indices (e.g. np.arange(1, ny-1))

        Returns
        -------
        u_analytical : velocity at each node (bounce-back corrected)
        """
        # xi = distance from bottom no-slip plane (y=0.5)
        xi = y - 0.5
        return self.Fx / (2.0 * self.nu) * xi * (self.H - xi)

    def l2_error(self) -> float:
        """L2 relative error vs. analytical Hagen-Poiseuille profile."""
...
def plot_profile(sim: PoiseuilleFlow, save_path: Path | None = None) -> None:
    ...
    fig.suptitle(
        f"D2Q9 LBM — Poiseuille Flow   Re={sim.Re:.1f}"
        f"   step={sim.step:,}\n"
...
def grid_refinement_study(
    ...
    Demonstrates second-order spatial accuracy of LBM + bounce-back BCs.
    """
    if ny_list is None:
    ...
    for ny in ny_list:
        ...
        dy_list.append(dy)
        err_list.append(err)
        print(f"  ny={ny:4d}  dy={dy:.4f}  L2={err:.3e}")
