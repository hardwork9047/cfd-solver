...
def _equilibrium_3d(
    ...
    for i in range(Q3):
        ...
        feq[i] = W3[i] * rho * (
            1.0 + cu / CS2_3 + 0.5 * cu**2 / CS2_3**2 - 0.5 * u2 / CS2_3
        )
    return feq
...
class LBMDEMSolver3D:
    """3D Lattice-Boltzmann solver using D3Q15 lattice with Lees-Edwards BC.

    Implements BGK collision and periodic streaming in x/z with Lees-Edwards
    shear boundary conditions in y.  Particle coupling is not included.
        ...
        if self.le_interpolation_order >= 3:
            try:
                ...
                for k in range(rolled.shape[1]):
                    result[:, k] = map_coordinates(
                        rolled[:, k], [x_dst], order=3, mode="wrap"
                    )
                return result
            except ImportError:
                pass
    ...
    def advance(self, n_steps: int = 1) -> None:
        ...
        Each step: collide → stream → Lees-Edwards BC correction.
